...
unsafe fn dist_avx2_i16(q_vec: __m256i, b_ptr: *const i16) -> i32 {
    ...
    let sum1 = _mm256_add_epi32(sq, _mm256_shuffle_epi32(sq, 0b01001110));
...
#[inline(always)]
fn min_dist_to_bbox_i16(q: &[i16; 16], bbox: &[i16; 32]) -> i32 {
    let mut d2 = 0i32;
    for i in 0..14 {
        let v = q[i];
        let min_v = bbox[i];
        let max_v = bbox[i + 16];
        if v < min_v {
            let diff = (min_v - v) as i32;
            d2 += diff * diff;
        } else if v > max_v {
            let diff = (v - max_v) as i32;
            d2 += diff * diff;
        }
    }
    d2
...
unsafe fn scan_cluster_aos(
    ...
    for _ in 0..blocks_count {
        ...
        let block_bbox: &[i16; 32] = bbox.try_into().unwrap();
        if min_dist_to_bbox_i16(q_i16, block_bbox) < top_dists[4] {
...
pub unsafe extern "C" fn search_vector(query_ptr: *const f32, force_deep: i32) -> i32 {
    ...
    let nprobe = if force_deep == 1 { 64 } else { 12 };
    
    if nprobe > 0 && nprobe <= sub.len() {
        sub.select_nth_unstable_by(nprobe - 1, |a, b| a.0.cmp(&b.0));
        let top_nprobe = &mut sub[0..nprobe];
        top_nprobe.sort_unstable_by(|a, b| a.0.cmp(&b.0));
    }
    ...
    for i in 0..nprobe {
        let ki = sub[i].1;
        if min_dist_to_bbox_i16(&q_i16, &bboxes[ki]) > top_dists[4] { continue; }
